    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <form action={savePlatformConfig} className="space-y-6">
        ...
        <section className="bg-white rounded-2xl border border-[var(--color-border)] p-6 space-y-4">
          <h2 className="font-semibold text-[var(--color-foreground)]">Commission plateforme</h2>
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                className="w-32 px-4 py-2.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-cream)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent"
              />
              <span className="text-sm text-[var(--color-muted-foreground)]">
                % prélevé sur chaque paiement
              </span>
            </div>
            <p className="text-xs text-[var(--color-muted-foreground)] mt-1.5">
              Actuellement : {(config.commissionRate * 100).toFixed(1)}% — sur un paiement de 10 000 FCFA, la plateforme perçoit {(10000 * config.commissionRate).toFixed(0)} FCFA
            </p>
          </div>
        </section>
